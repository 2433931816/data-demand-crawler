...
|--------|------|----------|
| 尚数网 | ✅ 已接入 | 公开 API，无需登录 |
| 北京国际大数据交易所 | ✅ 已接入 | API + Cookie |
| 上海数据交易所 | ✅ 已接入 | API + Cookie |
| 广州数据交易所 | ✅ 已接入 | API + Token |
| 杭州数据交易所 | ✅ 已接入 | HTML 解析（SSR），需 Cookie |

> 杭州数据交易所采用服务端渲染（SSR），数据直接嵌入 HTML，通过 BeautifulSoup 解析需求卡片。
