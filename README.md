...
本项目实现了对 **尚数网、北京、上海、广州、杭州、深圳、山东、湖南、郑州、贵阳、华东江苏、福建、安徽、北部湾** 等全国主要数据交易平台的数据需求/数据产品的每日自动抓取、清洗、存储与可视化展示。
...
- **自动抓取**：工作日 10:00 定时运行，支持 **14 个数据源**
...
| 9 | 华东江苏大数据交易中心 | ⚠️ 部分接入 | API + Cookie | ~50 条（共62条，分页限制） |
| 10 | 安徽数据交易所 | ✅ 稳定 | API + Cookie | ~11 条 |
| 11 | 福建大数据交易所 | ✅ 已接入 | API + Cookie | 需求广场暂无数据 |
| 12 | 北部湾大数据交易中心 | ✅ 稳定 | HTML 解析（无需登录） | ~20 条 |
